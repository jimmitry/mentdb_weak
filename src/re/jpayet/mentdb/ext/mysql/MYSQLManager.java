...
public class MYSQLManager {
	...
	public static void open(boolean newDatabase) throws Exception {
		...
		if (Start.MODE_NO_LIGHT) {
			...
			Connection recordConnection = Record2.connect();
			...
			if (newDatabase) {
				...
				try {
					...
					stm.execute(FileFx.load(null, "tools"+File.separator+"mentdb.sql"));
				} catch (Exception e) {
					System.out.println("Load mentdb.sql: Error: "+e.getMessage());
			...
			if (newDatabase) {
				...
				try {
					stm = recordConnection.createStatement();
					stm.execute(FileFx.load(null, "tools"+File.separator+"mentdb_h2.sql"));
				} catch (Exception e) {
					System.out.println("Load mentdb_h2.sql: Error: "+e.getMessage());
					try {stm.close();} catch (Exception f) {}
				}
				
				Record2.add("record", "jim[]", "{}");
	...
	public static void addLog(String script, String parent_pid, String pid, String msg, String status, String c_key, String c_val) throws Exception {
		...
		executeUpdate("INSERT INTO `logs`\n" + 
				...
				SQLManager.encode(c_val)+","
				+SQLManager.encode(Start.CLUSTER_NODENAME)+ ");", false);
	...
	public static void removeLog() throws Exception {
		...
		try {
			...
			Connection sqlConnection = Record2.connect();
			...
			//Get data
			stm = sqlConnection.createStatement();
			if (Start.MODE_NO_LIGHT) {
				...
				rs = stm.executeQuery("SELECT `logs`.`id`,\n" + 
						...
						"    `logs`.`c_val`,\n" + 
						"    `logs`.`nodename`\n" + 
						"FROM `logs` WHERE dtInsert<'"+DateFx.diff(DateFx.sysdate(), "DAY", ""+Start.LOG_RETENTION_DAYS)+"'"+" ORDER BY dtInsert");
			...
			while (rs.next()) {
				...
				sql.append("INSERT INTO `logs` " + 
						...
						SQLManager.encode(rs.getString(10))+");\n");
			...
			if (Start.MODE_NO_LIGHT) {
				executeUpdate("DELETE FROM `logs` WHERE dtInsert<'"+DateFx.diff(DateFx.sysdate(), "DAY", ""+Start.LOG_RETENTION_DAYS)+"'", true);
			} else {
				executeUpdate("DELETE FROM `logs` WHERE dtInsert<'"+DateFx.diff(DateFx.sysdate(), "DAY", ""+Start.LOG_RETENTION_DAYS)+"'", true);
			}
	...
	public static String execute(String query, boolean throwException, boolean getLastUpdate) throws Exception {
		...
		try {
			...
			//Execute query
			stm1 = sqlConnection.createStatement();
			if (getLastUpdate) {
			...
			try {
				...
				//Execute query
				stm2 = sqlConnection.createStatement();
				if (getLastUpdate) {
