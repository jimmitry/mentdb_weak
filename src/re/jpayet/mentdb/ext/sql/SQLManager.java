...
public class SQLManager {
	...
	public static void block_open(long sessionId, EnvManager env, String sqlId, String tablename, String selectQuery) throws Exception {
		...
		try {
			...
			//Get a statement and a resultset
			env.sql_block_stm = c.createStatement();
			env.sql_block_stm.setQueryTimeout(env.sql_query_timeout);
			env.sql_block_indexStatement[0] = SQLManager.addStatement(sessionId, env.sql_block_stm);
			env.sql_block_rs = env.sql_block_stm.executeQuery(selectQuery);
			env.sql_block_tablename = tablename;

		} catch (Exception f) {
			
			throw new Exception("Sorry, there is a SQL error ("+f.getMessage()+").");
			
		}
		
	}

	public static void block_close(long sessionId, EnvManager env) throws Exception {

		//Generate an error if the SQL id does not exist
		if (env.sql_block_stm==null) {

			throw new Exception("Sorry, the block manager is not open.");

		}
		
		if (env.sql_block_indexStatement[0]!=0) {
			
			SQLManager.deleteStatement(sessionId, env.sql_block_indexStatement[0]);
		...
		try {
			...
			//Close the statement
			env.sql_block_stm.close();

		} catch (Exception f) {}
		
		env.sql_block_stm = null;
		env.sql_block_indexStatement[0] = 0;
		env.sql_block_rs = null;
		env.sql_block_tablename = null;
		
	}

	@SuppressWarnings("unchecked")
	public static JSONObject block_to_json(long sessionId, EnvManager env, String nb_line) throws Exception {
		...
		JSONObject table = new JSONObject();
		table.put("table", env.sql_block_tablename);
		JSONArray columns = new JSONArray();
		table.put("columns", columns);
		JSONArray data = new JSONArray();
		table.put("data", data);
		...
		try {
			...
			i_nb_line = Integer.parseInt(nb_line);
			...
			throw new Exception("Sorry, nb block is not valid (number > 0).");
			
		}

		try {

			ResultSetMetaData meta = env.sql_block_rs.getMetaData();
			...
			while (env.sql_block_rs.next()) {
				...
				for(int i=0;i<meta.getColumnCount();i++) {
					...
					line.put((String) columns.get(i), env.sql_block_rs.getString(i+1));
		...
		try {
			...
			try {
				...
				while (rs.next()) {
					...
					session.sessionThreadAgent.current_function = "sql parse > next";
