...
public class Log {
	 ...
	 */
	public static void write(String scriptname, String message, String status, String c_key, String c_val, String parent_pid) throws Exception {
		
		//Log only if the parent_pid is not null
		if (parent_pid!=null) {
			
			MYSQLManager.addLog(scriptname, parent_pid, null, StringFx.substring(message, "0", ParameterManager.get_value("LOG_WRITE_MAX_SIZE_IN_STACK")), status, c_key, c_val);
	...
	public static String search(String status, String script, String c_key, String c_val, String msgFilter,
		...
		table.put("mql", "restricted log_search \""+status+"\" \n" + 
		...
		try {
			...
			while (rs.next()) {
				...
				String nodename = rs.getString(10);
				
				JSONObject line = new JSONObject();
				line.put("dtInsert", l_dtInsert);
				line.put("status", l_status);
				line.put("script", l_script);
				if (l_parent_pid!=null && !l_parent_pid.equals("")) line.put("parent_pid", "\""+nodename+"\" "+l_parent_pid);
				else line.put("parent_pid", l_parent_pid);
	...
	public static String activity(String dtMin, String dtMax, String grouptype) throws Exception {
		...
		try {
			...
			table.put("mql", "restricted show activity "+grouptype+"\n" + 
					"	\""+dtMin+"\"\n" + 
					"	\""+dtMax+"\"");
	...
	public static void get_pid_linked(String nodename, String pid, HashMap<String, Integer> alreadySearch) throws Exception {
		...
		try {
			...
			cmo = MYSQLManager.select("SELECT parent_pid, pid\n" + 
					"FROM `logs` WHERE (pid="+SQLManager.encode(pid)+" or parent_pid="+SQLManager.encode(pid)+") and nodename="+SQLManager.encode(nodename));
		...
		for(int i = 0; i<toSearch.size();i++) {
			...
			get_pid_linked(nodename, toSearch.get(i), alreadySearch);
	...
	@SuppressWarnings("unchecked")
	public static String show_time(String nodename, String pid) throws Exception {
		...
		table.put("mql", "restricted show_pid \""+nodename+"\" \""+pid+"\"");
		JSONArray data = new JSONArray();
		table.put("data", data);
		
		HashMap<String, Integer> pid_list = new HashMap<String, Integer>();
		get_pid_linked(nodename, pid, pid_list);
		...
		try {
			...
			cmo = MYSQLManager.select("SELECT dtInsert, "
					...
					"    `msg`,\n" + 
					"    `id`\n" + 
					"FROM `logs` WHERE (pid in ("+sql_pid+") or parent_pid in ("+sql_pid+")) and nodename="+SQLManager.encode(nodename)+" ORDER BY id;");
