...
public class PALinearRegression {
	...
	@SuppressWarnings("unchecked")
	public static JSONObject xy_scatter(String json, String title) throws Exception {
	...
	@SuppressWarnings("unchecked")
	public static JSONArray polynomial_curve_fit_get_coeff(String data, String degree) throws Exception {
		
		WeightedObservedPoints obs = new WeightedObservedPoints();
		JSONArray a = (JSONArray) JsonManager.load(data);
		
		JSONArray first = null;
        
        for(int i=0;i<a.size();i++) {
        	
        		JSONArray aa = (JSONArray) a.get(i);
        		obs.add(Double.parseDouble(""+aa.get(0)),  Double.parseDouble(""+aa.get(1)));
        ...
        PolynomialCurveFitter fitter = PolynomialCurveFitter.create(Integer.parseInt(degree));
        fitter.withStartPoint(new double[] {Double.parseDouble(""+first.get(0)),  Double.parseDouble(""+first.get(1))});
	...
	public static String polynomial_curve_fit_eval(String coeff_array, String x) throws Exception {
		
		JSONArray coeff_a = (JSONArray) JsonManager.load(coeff_array);
		double[] coeff = new double[coeff_a.size()];
		
		for(int i=0;i<coeff_a.size();i++) {
			
			coeff[i]=Double.parseDouble(""+coeff_a.get(i));
			
		}
		
		PolynomialFunction func = new PolynomialFunction(coeff);

		return ""+func.value(Double.parseDouble(x));
		
	}

	@SuppressWarnings("unchecked")
	public static JSONArray polynomial_curve_fit_eval_incr(String coeff_array, String x_min, String x_max, String x_incr) throws Exception {
		...
		double min = Double.parseDouble(x_min);
		double max = Double.parseDouble(x_max);
		double incr = Double.parseDouble(x_incr);
		
		for(double i=min;i<=max;i+=incr) {
			
			JSONArray row = new JSONArray();
			row.add(i);
			row.add(func.value(i));
			result.add(row);
