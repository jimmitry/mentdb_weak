...
public class StimulationManager {
	...
	public static void init(long sessionId) throws Exception {
		...
		Record2.add("record", "STIM_WAIT[]", stim_wait.toJSONString());
		Record2.add("record", "TASKS[]", new JSONObject().toJSONString());
		
	}
	
	@SuppressWarnings("unchecked")
	public static void waiting_push(String targetUser, String sentence) throws Exception {
		
		JSONObject o = Record2.getNode("STIM_WAIT[]");
		...
		Record2.update("STIM_WAIT[]", o.toJSONString());
	...
	public static JSONArray waiting_pull(long sessionId) throws Exception {
		...
		JSONObject obj = Record2.getNode("STIM_WAIT[]");
		...
		Record2.update("STIM_WAIT[]", obj.toJSONString());
	...
	@SuppressWarnings("unchecked")
	public static void reset(String user, EnvManager env) throws Exception {
	...
	public static void saveTasks(long sessionId) throws Exception {
		...
		Record2.update("TASKS[]", StimulationManager.ai_tasks.toJSONString());
	...
	public static String call(boolean initialization, String targetUser, String str, EnvManager env, SessionThread session) throws Exception {
		...
		if (initialization) {
			...
			return CommandManager.executeAllCommands(session, Misc.splitCommand("include \"brain.internal.stimulate.user.post\" \"[ai_str]\" \""+str.replace("\"", "\\\"")+"\"  \"[ai_targetUser]\" \""+targetUser.replace("\"", "\\\"")+"\";"), env, null, null);
			
		} else {
			
			return CommandManager.executeAllCommands(session, Misc.splitCommand("include \"brain.internal.click.post\";"), env, null, null);
	...
	@SuppressWarnings("unchecked")
	public static JSONObject StimulationValue(SessionThread session, EnvManager env, String value, int strType, boolean showTypes) throws Exception {
	...
	//Split without empty
	@SuppressWarnings("unchecked")
	public static JSONArray splitStimulation(SessionThread session, EnvManager env, String str, boolean showTypes) throws Exception {
		...
		while (!tmpstr.toString().equals("")) {
			...
			case "?": case ",": case ".": case ";": case "!": case ":": 
				...
				((JSONArray) ((JSONObject) commands.get(commands.size()-1)).get("s")).add(StimulationValue(session, env, tmpstr.substring(0, 1), 9, showTypes));
			...
			case "\"": 
				...
				if (endIndex==-1) {
					...
					((JSONArray) ((JSONObject) commands.get(commands.size()-1)).get("s")).add(StimulationValue(session, env, tmptmpstr.replace("\\\"", "\""), 1, showTypes));
				...
				if (endIndex==-1) {
					...
					if (!tmptmpstr.equals("")) {
						...
						for(int sp = 0;sp<split_word.size();sp++) {
							((JSONArray) ((JSONObject) commands.get(commands.size()-1)).get("s")).add(StimulationValue(session, env, ""+split_word.get(sp), 0, showTypes));
						}
					}
