...
public class SearchEngine {
	...
	public static JSONArray execute(JSONObject context, String text, String lang, boolean searchPunctuation, SessionThread session, EnvManager env) throws Exception {
		...
		JSONArray sentenceArray = (JSONArray) ((JSONObject) StimulationManager.splitStimulation(session, env, text, true).get(0)).get("s");
		
		return execute(context, text, session.user, "", lang, input_text, sentenceArray, "*", thoughtFilter, searchPunctuation, session, env, new JSONObject());
		
	}
	
	//Search relations
	@SuppressWarnings("unchecked")
	public static JSONArray search(String text, String lang, boolean searchPunctuation, SessionThread session, EnvManager env) throws Exception {
		...
		return execute(new JSONObject(), text, session.user, "", lang, input_text, sentenceArray, "*", thoughtFilter, searchPunctuation, session, env, new JSONObject());
		
	}
	
	//Search
	@SuppressWarnings("unchecked")
	public static JSONArray execute(JSONObject context, String inputText, String targetUser, String filter_search, String g_l, JSONArray inputVector, JSONArray sentenceArray, String searchTypeList, JSONArray thoughtFilter, boolean searchPunctuation, SessionThread session, EnvManager env, JSONObject n_input) throws Exception {
		...
		for(int i=0;i<inputVector.size();i++) {
			...
			if (searchPunctuation || ("?,.;!:".indexOf(wordName)==-1)) {
				allThoughtString.add(getAllThoughts(inputVector, i, wordName, allThoughts, mpa, thoughtFilter.get(i)+""));
			}
        }
				...
				for(Entry<String, Vector<String>> e: allThoughtsL.entrySet()) {
					...
					for(int i=0;i<vectorOfThoughts.size();i++) {
						...
						for(int z=0;z<typeList.length();z++) {
							...
							searchFromThought(context, g_l, thoughtId, sentenceArray, ""+typeList.charAt(z), i+1, alreadyUsed, allThoughtString, concentration, typeList, session, env, filter_search, context_search, n_input);
	...
	@SuppressWarnings("unchecked")
	public static HashMap<String, Double> getAllThoughts(JSONArray inputVector, int index, String wordName, HashMap<String, HashMap<String, Vector<String>>> allThoughts, double mpa, String thoughtFilter) throws Exception {
		
		JSONArray languages = new JSONArray();
		HashMap<String, Double> allThoughtString = new HashMap<String, Double>();
		
		try {
			languages = WordManager.showLanguages(wordName);
		} catch (Exception e) {};
		...
		if (languages.size()==0 || (languages.size()==1 && (languages.get(0)+"").equals("io"))) {
			...
			JSONArray a = WordManager.searchLevenshteinDistance(wordName, ConcentrationManager.getConcentrationDepth("C[symbol]"), 1);
		...
		try {
			...
			for(int i=0;i<languages.size();i++) {
				...
				String filteredThoughts = ThoughtManager.search(wordName, curLang, thoughtFilter);
				...
				String thoughtId = ExperienceManagerL1.getFirstTabLink("W["+wordName+"]", curLang, true);
				int depth = 1;
				
				//Parse all tab links
				while (thoughtId!=null) {
					
					JSONObject recNode = Record2.getNode(thoughtId);
					...
					thoughtId = ExperienceManagerL1.getDownTabLink("W["+wordName+"]", thoughtId, curLang, true);
	...
	public static void searchFromThought(JSONObject context, String g_l, String thoughtId, JSONArray sentenceArray, String type, int depth, HashMap<String, Double> alreadyUsed, Vector<HashMap<String, Double>> allThoughtString, int concentration, String typeList, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {
		
		//Get the first
		String tl = ExperienceManagerL2.getFirstTabLink(thoughtId, type);
		...
		while(tl!=null && depth<=concentration) {
			...
			searchInCircle(context, g_l, relationId, sentenceArray, typeList, allThoughtString, alreadyUsed, concentration-depth, session, env, filter_search, context_search, n_input);
			
			//Get the next down node
			tl = ExperienceManagerL2.getDownTabLink(thoughtId, tl, type);
	...
	public static void searchInCircle(JSONObject context, String g_l, String r, JSONArray sentenceArray, String typeFilter, Vector<HashMap<String, Double>> allThoughtString, HashMap<String, Double> alreadyUsed, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {
		
		JSONArray circle_ids = CircleManager.getIds("r", r);
		for(int i=0;i<circle_ids.size();i++) {
			
			String rc = (String) circle_ids.get(i);
			
			if (!alreadyUsed.containsKey(rc)) {
				checker(context, g_l, rc, sentenceArray, typeFilter, allThoughtString, alreadyUsed, depth, session, env, filter_search, context_search, n_input);
			}
	...
	public static void checker(JSONObject context, String g_l, String r, JSONArray sentenceArray, String typeFilter, Vector<HashMap<String, Double>> allThoughtString, HashMap<String, Double> alreadyUsed, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {
		
		check_relation(context, g_l, r, sentenceArray, typeFilter, allThoughtString, alreadyUsed, depth, session, env, filter_search, context_search, n_input);
		
		check_parent(g_l, r, sentenceArray, typeFilter, allThoughtString, alreadyUsed, depth, session, env, filter_search, context_search, n_input);
		
		check_in_relation(context, g_l, r, sentenceArray, typeFilter, allThoughtString, alreadyUsed, depth, session, env, filter_search, context_search, n_input);
		
		check_sub_relation(context, g_l, r, sentenceArray, typeFilter, allThoughtString, alreadyUsed, depth, session, env, filter_search, context_search, n_input);
		
	}
	
	public static void check_relation(JSONObject context, String g_l, String r, JSONArray sentenceArray, String typeFilter, Vector<HashMap<String, Double>> allThoughtString, HashMap<String, Double> alreadyUsed, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {
		
		if (typeFilter.indexOf(r.charAt(1)+"")>-1) {
			
			JSONObject obj = Record2.getNode(r);
			...
			double calcul = cognitiveDistance(weight, g_l, r, (String) obj.get("sty"), r_l, ct, sentenceArray, rlth, allThoughtString, depth, session, env, filter_search, context_search, n_input);
			
			if (!alreadyUsed.containsKey(r)) {
				try {
					calcul += 1000.0/(Double.parseDouble(""+context.get(ct)));
				} catch (Exception e) {};
	...
	public static void check_parent(String g_l, String r, JSONArray sentenceArray, String typeFilter, Vector<HashMap<String, Double>> allThoughtString, HashMap<String, Double> alreadyUsed, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {
		...
		if (parentId!=null && !alreadyUsed.containsKey(parentId) && typeFilter.indexOf(parentId.charAt(1)+"")>-1) {
			...
			double calcul = cognitiveDistance(weight, g_l, parentId, (String) obj.get("sty"), r_l, ct, sentenceArray, rlth, allThoughtString, depth, session, env, filter_search, context_search, n_input);
		...
		if (parentId!=null) {
			...
			check_parent(g_l, parentId, sentenceArray, typeFilter, allThoughtString, alreadyUsed, depth-1, session, env, filter_search, context_search, n_input);
		
		}
		
	}
	
	public static void check_in_relation(JSONObject context, String g_l, String r, JSONArray sentenceArray, String typeFilter, Vector<HashMap<String, Double>> allThoughtString, HashMap<String, Double> alreadyUsed, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {
		...
		for(int iBranch=0;iBranch<RelationManager.allTypeList.length();iBranch++) {
			...
			String tl = ExperienceManagerL2.getFirstTabLink(r, type);
			...
			while(tl!=null && ii>0) {
				...
				//Search in circle
				searchInCircle(context, g_l, relationId, sentenceArray, typeFilter, allThoughtString, alreadyUsed, ii, session, env, filter_search, context_search, n_input);
				
				ii=ii-1;
				
				//Get the next down node
				tl = ExperienceManagerL2.getDownTabLink(r, tl, type);
	...
	public static void check_sub_relation(JSONObject context, String g_l, String r, JSONArray sentenceArray, String typeFilter, Vector<HashMap<String, Double>> allThoughtString, HashMap<String, Double> alreadyUsed, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws Exception {

		JSONArray tabLinkIds = RelationManager.listTabLinks(r);
		for(int i=0;i<tabLinkIds.size();i++) {
			...
			for(int iBranch=0;iBranch<RelationManager.allTypeList.length();iBranch++) {
				...
				String type = RelationManager.allTypeList.charAt(iBranch)+"";
				String tl = ExperienceManagerL2.getFirstTabLink(currentRelationTabLink, type);
				...
				while(tl!=null && ii>0) {
					...
					//Get the next down node
					tl = ExperienceManagerL2.getDownTabLink(currentRelationTabLink, tl, type);
	...
	public static double cognitiveDistance(double weight, String g_l, String r, String SUBTYPE, String r_l, String ct, JSONArray sentenceArray, String rlth, Vector<HashMap<String, Double>> allThoughtString, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws NumberFormatException, Exception {
		...
		if (result>0) {
			...
			if (rlth.indexOf("TH[]")>-1) return percent+result+next+depth+RelationManager.get_TH_TypeWeightValue(g_l, r, sentenceArray, session, env, n_input);
	...
	public static double simpleCognitiveDistance(double weight, String g_l, String r, String r_l, String ct, JSONArray sentenceArray, String rlth, Vector<HashMap<String, Double>> allThoughtString, int depth, SessionThread session, EnvManager env, String filter_search, JSONObject context_search, JSONObject n_input) throws NumberFormatException, Exception {
		...
		if (result>0) {
			...
			if (rlth.indexOf("TH[]")>-1) return percent+result+depth+RelationManager.get_TH_TypeWeightValue(g_l, r, sentenceArray, session, env, n_input);
