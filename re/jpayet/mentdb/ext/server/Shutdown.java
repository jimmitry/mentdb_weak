	...
	public static void main(String args[]) throws Exception {
		...
		try {
			...
			connector = new MentDBConnector("localhost", 
					Integer.parseInt(FileFx.ini("conf/server.conf", "DATABASE", "SERVER_PORT")), 10000, 10000, 
					FileFx.ini("conf/server.conf", "ADMIN", "SERVER_ENCRYPT_KEY"));
			
			//User connection
			if (connector.connect("admin", FileFx.ini("conf/server.conf", "ADMIN", "PWD"))) {
