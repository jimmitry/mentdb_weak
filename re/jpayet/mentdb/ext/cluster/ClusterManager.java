...
public class ClusterManager {
	...
	public static void init(long sessionId) throws Exception {
		...
		Record.add(sessionId, "CLUSTER[]", (new JSONObject()).toJSONString());
		Record.add(sessionId, "SIGNAL[]", (new JSONObject()).toJSONString());

		allclusters_obj = (JSONObject) JsonManager.load(Record.getNode(sessionId, "CLUSTER[]").toJSONString());
		allclusters_array = transform_cluster_obj_to_array(allclusters_obj);
		
		allsignals_obj = Record.getNode(sessionId, "SIGNAL[]");
	...
	public static void add(long sessionId, String cluster_id) throws Exception {
		...
		Record.update(sessionId, "CLUSTER[]", allclusters_obj.toJSONString());
		...
		for(int i=0;i<nodes.size();i++) {
			...
			if (((String) n.get("in_the_cluster")).equals("1")) {
				...
				if (now_l>node_l+Start.CLUSTER_LIFE_BEFORE_EXPULSION) {
					...
					ClusterManager.node_expels(sessionId, cluster_id, ""+n.get("node_id"), "Sorry, the node '"+n.get("node_id")+"' took too long to give sign of life.");
	...
	@SuppressWarnings("unchecked")
	public static void node_expels(long sessionId, String cluster_id, String node_id, String error) throws Exception {
	...
	public static void signal_set(long sessionId, String cluster_id, String node_id, String hostname, String port, String user, String user_key, String password, String connectTimeout, String readTimeout, String mql_signal) throws Exception {
		...
		Record.update(sessionId, "SIGNAL[]", allsignals_obj.toJSONString());
