...
public class Doc_Mentalese_SOA {
	...
	public static void init(LinkedHashMap<String, Vector<MQLDocumentation>> functions,
		...
		page_description.put("Script", "<img src='images/p.png' style='vertical-align: middle;'>MQL functions can be saved in a script."+
				"<br><img src='images/p.png' style='vertical-align: middle;'>You can call these scripts inside the mental processes."+
		...
		functions.get("Script").add(mql);
		mql = new MQLDocumentation(true, "script export_group", "To export all scripts from a group to file", "script export_group \"group\"", "Scripts exported with successfully.", null, null, null, null, false, "");
		mql.addParam(new MQLParam("group", "The group name", "string", true));
		functions.get("Script").add(mql);
		mql = new MQLDocumentation(true, "script import_group", "To import all scripts from file to a group", "script import_group \"group\"", "Scripts imported with successfully.", null, null, null, null, false, "");
		mql.addParam(new MQLParam("group", "The group name", "string", true));
		functions.get("Script").add(mql);
		mql = new MQLDocumentation(true, "script get_all", "To get all scripts", "script get_all;1", "Warning, The scripts are loaded into the editor ...", null, null, null, null, false, "");
		functions.get("Script").add(mql);
		...
		mql.addParam(new MQLParam("separator", "The separator for word creation", "string", false));
		...
		mql.addParam(new MQLParam("password", "The password", "string", true));
		functions.get("User").add(mql);
		mql = new MQLDocumentation(true, "user force create", "To create a new user (force if the group already exist)", "user force create \"bob2\" \"pwd\"", "User bob2 created with successful.", "user force create \"luc-yann2\" \"pwd\" \"-\"", "User luc-yann2 created with successful.", null, null, false, "user force create|insert|add");
		mql.addParam(new MQLParam("login", "The login", "string", true));
		mql.addParam(new MQLParam("password", "The password", "string", true));
		mql.addParam(new MQLParam("separator", "The separator for word creation", "string", false));
		functions.get("User").add(mql);
		...
		mql.addParam(new MQLParam("login", "The login", "string", true));
		functions.get("User").add(mql);
		mql = new MQLDocumentation(true, "user disable", "To disable a user", "user disable \"bob\"", "1", null, null, null, null, false, "");
		mql.addParam(new MQLParam("login", "The login", "string", true));
		functions.get("User").add(mql);
