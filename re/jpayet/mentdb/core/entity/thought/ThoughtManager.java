...
public class ThoughtManager {
	...
	public static JSONObject pearlNecklaceRelations(long sessionId, String key) throws Exception {
		...
		JSONObject rec = Record.getNode(sessionId, key);
		...
		if (key.startsWith("TH[")) {
			...
			JSONArray c = CircleManager.getIds(sessionId, "th", key);
			...
			ThoughtManager.findParent(sessionId, key, nodes, edges, autoincrement, lastKey);
			ThoughtManager.findCircle(sessionId, key, nodes, edges, autoincrement, lastKey);
			
			JSONArray thoughts = RelationManager.showThoughtsRecursivelyRLTH(sessionId, key);
			...
			for(int i=0;i<thoughts.size();i++) {
				...
				JSONArray c = CircleManager.getIds(sessionId, "th", thoughtId);
				for(int u=0;u<c.size();u++) {
		...
		for(int iType=0;iType<RelationManager.allTypeList.length();iType++) {
			...
			branch(sessionId, key, RelationManager.allTypeList.charAt(iType)+"", nodes, edges, autoincrement, lastKey, concentration);
			
		}
		
		if (key.startsWith("R")) {
			
			JSONArray tabLinks = RelationManager.listTabLinks(sessionId, key);
			...
			for(int i=0;i<tabLinks.size();i++) {
				...
				for(int iType=0;iType<RelationManager.allTypeList.length();iType++) {
					...
					branch(sessionId, curTabLink, RelationManager.allTypeList.charAt(iType)+"", nodes, edges, autoincrement, lastKey, concentration);
	...
	@SuppressWarnings("unchecked")
	public static void findCircle(long sessionId, String key, JSONObject nodes, JSONArray edges, int[] autoincrement, String lastKey) throws Exception {
		
		JSONObject bd = Record.getNode(sessionId, key);
		...
		if (circle_id==null) currentCircle =  new JSONObject();
			...
			currentCircle =  Record.getNode(sessionId, circle_id);
	...
	@SuppressWarnings("unchecked")
	public static void findParent(long sessionId, String key, JSONObject nodes, JSONArray edges, int[] autoincrement, String lastKey) throws Exception {
		...
		if (parentId!=null && !parentId.equals("")) {
			...
			findParent(sessionId, parentId, nodes, edges, autoincrement, lastKey);
	...
	@SuppressWarnings("unchecked")
	public static void branch(long sessionId, String key, String branch, JSONObject nodes, JSONArray edges, int[] autoincrement, String lastKey, int depth) throws Exception {
		
		JSONArray list = RelationManager.showTabLinks(sessionId, key, branch, depth);
	...
	//Add a new thought
	public static String create(long sessionId, String wordName, String separator, String lang, String lock_translation, SessionThread session, EnvManager env) throws Exception {
		
		if (separator==null || separator.equals("")) {
			
			String thoughtId = createThought(sessionId, wordName, lang, lock_translation);
			...
			String thoughtId = createThoughtComposed(sessionId, wordName, separator, lang, words, lock_translation, session, env);
	...
	//Add a new thought
	@SuppressWarnings("unchecked")
	public static String createThought(long sessionId, String wordName, String lang, String lock_translation) throws Exception {
		...
		JSONObject rec = Record.getNode(sessionId, "W["+wordName+"]");
		...
		if (wordName.equals("quelque chose") && lang.equals("fr") && !NodeManager.existNode(sessionId, "TH[]")) {
			
			thoughtId = "TH[]";
			
		} else {
			
			thoughtId = "TH["+SequenceManager.incr(sessionId, "thought")+"]";
			while (Record.getNode(sessionId, thoughtId)!=null) {
		...
		ExperienceManagerL1.addNodeTabLink(sessionId, "W["+wordName+"]", thoughtId, lang, "word", true, true, 0);
		
		JSONObject bd = Record.getNode(sessionId, thoughtId);
		...
		Record.update(sessionId, thoughtId, bd.toJSONString());
	...
	@SuppressWarnings("unchecked")
	public static String createThoughtComposed(long sessionId, String wordName, String separator, String lang, String[] words, String lock_translation, SessionThread session, EnvManager env) throws Exception {
		...
		for(int j=1;j<allThoughtIds.size();j++) {
			ThoughtManager.merge(sessionId, "th", allThoughtIds.get(0), allThoughtIds.get(j), session, env);
		}
		
		String thoughtId = "";
		
		if (gWordName.equals("quelque chose") && lang.equals("fr") && !NodeManager.existNode(sessionId, "TH[]")) {
		...
		ExperienceManagerL1.addNodeTabLink(sessionId, "W["+gWordName+"]", thoughtId, lang, "word", true, true, 0);
		...
		ThoughtManager.merge(sessionId, "th", thoughtId, allThoughtIds.get(0), session, env);
	...
	public static String search(long sessionId, String wordName, String lang, String typeFilter) throws Exception {
		...
		String currentTabLink = ExperienceManagerL1.getFirstTabLink(sessionId, "W["+wordName+"]", lang, true);
		...
		while (currentTabLink!=null) {
			...
			if (typeFilter==null || typeFilter.equals("")) {
				...
				for(int i=0;i<filters.length;i++) {
					...
					JSONObject p_type_node = Record.getNode(sessionId, "DT["+currentFilterType+"]");
					...
					selectTheThought = search_type_boolean(sessionId, currentThoughtId, PARENT_TYPE);
				...
				if (selectTheThought) {
					...
					result += search_type(sessionId, currentThoughtId, filters, "U");
					result += search_type(sessionId, currentThoughtId, filters, "A");
					
				}
				
			}
				
			currentTabLink = ExperienceManagerL1.getDownTabLink(sessionId, "W["+wordName+"]", currentTabLink, lang, true);
		}
	...
	public static String search_type(long sessionId, String currentThoughtId, String[] filters, String r_type) throws Exception {
		
		JSONArray relations = RelationManager.showTabLinks(sessionId, currentThoughtId, r_type, ConcentrationManager.getConcentrationDepth("C[relation]"));
		...
		for(int j=0;j<relations.size();j++) {
			...
			for(int i=0;i<filters.length;i++) {
				...
				selectTheThought = search_type_boolean(sessionId, currentRelation, PARENT_TYPE);
	...
	@SuppressWarnings("unchecked")
	public static double probability_in_words(long sessionId, String ths, String words) throws Exception {
		...
		for(int i=1;i<=nbWord;i++) {
			...
			if (NodeManager.existNode(sessionId, "W["+curWord+"]")) {
				JSONArray a = ThoughtManager.list(sessionId, curWord);
		...
		for(int i=1;i<=nbThought;i++) {
			...
			if (curTh.startsWith("TH")) {
				...
				JSONArray c = CircleManager.getIds(sessionId, "th", curTh);
				...
				JSONArray c = CircleManager.getIds(sessionId, "r", curTh);
				
				for(int z=0;z<c.size();z++) {
					
					String r = c.get(z)+"";
					
					String[] thoughts = ((String) Record.getNode(sessionId, r).get("rlth")).split(" ");
	...
	public static boolean search_type_boolean(long sessionId, String target, String PARENT_TYPE) throws Exception {
		
		boolean selectRelation = false;
		
		//Search all type relations for the current thought id
		JSONArray relationTypes = RelationManager.showTabLinks(sessionId, target, "T", ConcentrationManager.getConcentrationDepth("C[relation]"));
		...
		for(int j=0;j<relationTypes.size();j++) {
			...
			String CUR_PARENT_TYPE = (String) Record.getNode(sessionId, currentRelationType).get("p");
	...
	public static JSONArray list(long sessionId, String wordName, String lang) throws Exception {
		...
		return ExperienceManagerL1.getAllNodeTabLinks(sessionId, "W["+wordName+"]", lang);

	}
	
	//Get all thoughts linked with a word
	@SuppressWarnings("unchecked")
	public static JSONArray list(long sessionId, String wordName) throws Exception {
		...
		for(int i=0;i<languages.size();i++) {
			...
			try {
				...
				JSONArray array = ExperienceManagerL1.getAllNodeTabLinksForStimulation(sessionId, "W["+wordName+"]", lang);
	...
	@SuppressWarnings("unchecked")
	public static JSONArray getWords(long sessionId, String thoughtId) throws Exception {
		...
		if (thoughtId==null) {
			...
			for(int i=0;i<cir.size();i++) {
				...
				JSONObject bwTmp = Record.getNode(sessionId, th);
	...
	//Get words from thought
	@SuppressWarnings("unchecked")
	public static JSONArray getWords(long sessionId, String thoughtId, String lang, Vector<MQLValue> inputVector) throws Exception {
	...
	//Get words from thought
	@SuppressWarnings("unchecked")
	public static JSONArray getWordsForTranslation(long sessionId, String thoughtId, String lang, Vector<MQLValue> inputVector, String lock_lang) throws Exception {
		...
		if (thoughtId==null) {
			...
			JSONObject bwTmp = Record.getNode(sessionId, thoughtId);
			...
			if (ltr_obj.containsKey(lock_lang)) {
				...
				for(int i=0;i<cir.size();i++) {
					...
					bwTmp = Record.getNode(sessionId, th);
	...
	//Stimulate relations
	@SuppressWarnings("unchecked")
	public static void stimulateRelations(long sessionId, String thoughtId, JSONArray rels, EnvManager env, SessionThread session) throws Exception {
		...
		JSONObject recThought = Record.getNode(sessionId, thoughtId);
		...
		//Get all relations
		JSONArray relations = RelationManager.showTabLinks(sessionId, thoughtId, "L", ConcentrationManager.getConcentrationDepth("C[relation]"));
		...
		for(int i = relations.size()-1;i>=0;i--) {
			...
			RelationManager.stimulate(sessionId, relationId, true);

			JSONObject bd = Record.getNode(sessionId, relationId);
			String l = ""+bd.get("l");
			
			JSONArray words = RelationManager.showWords(sessionId, relationId, l, new Vector<MQLValue>(), env, session);
			...
			for(int j=0;j<words.size();j++) {
				...
				for(int k=0;k<cur.size();k++) {
					...
					for(int z=0;z<wordList.length;z++) {
						WordManager.stimulate(sessionId, wordList[z], lang);
					}
	...
	//Stimulate a thought
	@SuppressWarnings("unchecked")
	public static void stimulate(long sessionId, String thoughtId) throws Exception {
		...
		JSONObject th = Record.getNode(sessionId, thoughtId);
		...
		//Stimulate all words
		JSONArray words = ThoughtManager.getWords(sessionId, thoughtId);
		...
		for(int i=0;i<words.size();i++) {
			...
			WordManager.stimulate(sessionId, str_w.substring(2, str_w.length()-1), (String) w.get("lang"));
		...
		for(String entry : entrySet) {
			...
			//Delete the node
			ExperienceManagerL1.deleteNodeTabLink(sessionId, word, thoughtId, lang);
			...
			Record.update(sessionId, thoughtId, recThought.toJSONString());
			
			//Recreate the thought
			ExperienceManagerL1.mergeNodeTabLink(sessionId, word, thoughtId, lang, "word", true, false);
	...
	//Merge two thought
	public static void merge(long sessionId, String level, String thoughtId1, String thoughtId2, SessionThread session, EnvManager env) throws Exception {

		CircleManager.mergeCircle(sessionId, level, thoughtId1, thoughtId2, env, session);

	}
	
	//Delete a thought
	public static void delete(long sessionId, String thoughtId) throws Exception {
		...
		for (Object key : ud.keySet()) {
			...
			ExperienceManagerL1.deleteNodeTabLink(sessionId, keyStr, thoughtId, lang);
			
			//Search in the word if there is thought
			JSONObject w = Record.getNode(sessionId, keyStr);
			...
			if (b) {
				...
				for(int i=0;i<keyStr.length();i++) {
					...
					ExperienceManagerL1.deleteNodeTabLink(sessionId, "S["+currentLetter+"]", "TL["+StringFx.str_to_hex(keyStr)+" "+i+" "+lang+"]", lang);
					
				}
				
				Record.remove(sessionId, "W["+keyStr+"]");
	...
	//Delete a thought
	public static void deleteLang(long sessionId, String thoughtId, String searchLang) throws Exception {
	...
	//Delete a thought
	public static void deleteWord(long sessionId, String thoughtId, String searchWord) throws Exception {
	...
	//Delete a thought
	public static void delete(long sessionId, String wordName, String thoughtId, String lang) throws Exception {
		...
		ExperienceManagerL1.deleteNodeTabLink(sessionId, "W["+wordName+"]", thoughtId, lang);
