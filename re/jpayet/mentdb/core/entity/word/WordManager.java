...
public class WordManager {
	...
	public static JSONObject pearlNecklaceWord(long sessionId, String wordName) throws Exception {
		...
		for(String lang : entrySet) {
			...
			JSONArray list = ThoughtManager.list(sessionId, wordName, lang);
	...
	public static boolean exist(long sessionId, String wordName, String lang) throws Exception {
	...
	@SuppressWarnings("unchecked")
	public static JSONArray getLangProbability(long sessionId, Vector<MQLValue> inputVector) throws Exception {
		...
		for(int i=0;i<inputVector.size();i++) {
			...
			JSONObject rec = Record.getNode(sessionId, "W["+w.toLowerCase()+"]");
			if (rec!=null) {
				...
				rec = Record.getNode(sessionId, "W["+StringFx.first_letter_upper(w)+"]");
				if (rec!=null) {
					JSONObject obj = ((JSONObject) rec.get("fl"));
					for(Object o : obj.keySet()) {
						String k = (String) o;
	...
	public static void import_from_text(long sessionId, String text, String separator, String lang, String excludedChars, SessionThread session, EnvManager env) throws Exception {
		
		text = text.replace("\r\n", " ").replace("\r", " ").replace("\n", " ");
		
		text = StringFx.trim(text);
		
		for(int i=0;i<excludedChars.length();i++) {
			
			text = text.replace(""+excludedChars.charAt(i), "");
		...
		for(int i=0;i<tab.length;i++) {
			...
			String wordName = tab[i];
			
			if (wordName.indexOf(separator)>-1) create(sessionId, wordName, separator, lang, "0", session, env);
			else create(sessionId, wordName, null, lang, "0", session, env);
	...
	public static String create(long sessionId, String wordName, String separator, String lang, String lock_translation, SessionThread session, EnvManager env) throws Exception {
		...
		if (separator==null || separator.equals("")) {
			...
			return createWord(sessionId, wordName, lang, lock_translation, session, env);
			...
			return createWordComposed(sessionId, wordName, separator, lang, words, lock_translation, session, env);
	...
	//Add a new word
	@SuppressWarnings("unchecked")
	public static String createWord(long sessionId, String wordName, String lang, String lock_translation, SessionThread session, EnvManager env) throws Exception {
		...
		if (rec!=null) {
			...
			if (((JSONObject) rec.get("fl")).containsKey(lang)) {
				...
				return WordManager.firstTabLink(sessionId, wordName, lang);
				...
				for(int i=0;i<wordName.length();i++) {
					...
					ExperienceManagerL1.addNodeTabLink(sessionId, "S["+currentLetter+"]", "TL["+StringFx.str_to_hex(wordName)+" "+i+" "+lang+"]", lang, "symbol", false, false, 0);
		
				}

				Record.update(sessionId, "W["+wordName+"]", rec.toJSONString());
				
				return ThoughtManager.create(sessionId, wordName, "", lang, lock_translation, session, env);
			...
			//Add the record
			Record.add(sessionId, "W["+wordName+"]", bw.dataNode.toJSONString());
	...
	@SuppressWarnings("unchecked")
	public static String createWordComposed(long sessionId, String wordName, String separator, String lang, String[] words, String lock_translation, SessionThread session, EnvManager env) throws Exception {
		...
		//Try to get the record
		JSONObject recGWord = Record.getNode(sessionId, "W["+wordName+"]");
		...
		if (recGWord!=null && ((JSONObject) recGWord.get("fl")).containsKey(lang)) {
			...
			for(int i=0;i<gWordName.length();i++) {
				...
				ExperienceManagerL1.addNodeTabLink(sessionId, "S["+currentLetter+"]", "TL["+StringFx.str_to_hex(gWordName)+" "+i+" "+lang+"]", lang, "symbol", false, false, 0);

			}
			
			//Add the record
			Record.add(sessionId, "W["+gWordName+"]", bw.dataNode.toJSONString());

			return ThoughtManager.create(sessionId, gWordName, separator, lang, lock_translation, session, env);
	...
	//Stimulate a word
	@SuppressWarnings("unchecked")
	public static void stimulate(long sessionId, String wordName, String lang) throws Exception {
		...
		if (rec==null) {
			...
			if (!((JSONObject) rec.get("fl")).containsKey(lang)) {
				...
				for(int i=0;i<wordName.length();i++) {
					...
					SymbolManager.stimulate(sessionId, "TL["+StringFx.str_to_hex(wordName)+" "+i+" "+lang+"]");
	...
	//Search words
	@SuppressWarnings("unchecked")
	public static JSONArray searchLevenshteinDistance(long sessionId, String symbolChars, int concentrationDepth, int orderDirection, String lang) throws Exception {
		...
		for(int i=0;i<symbolChars.length();i++) {
			...
			if ("<([{\\^-=$!|]})?*+.>".indexOf(currentSymbolChar)==-1) {
				...
				String firstTL = ExperienceManagerL1.getFirstTabLink(sessionId, "S["+currentSymbolChar+"]", lang, true);

				if (firstTL!=null) {

					//Handle the first tab link
					JSONObject bd = Record.getNode(sessionId, firstTL);
					...
					while (downTL!=null && iDepth<=concentrationDepth) {
						...
						bd = Record.getNode(sessionId, downTL+"");
	...
	//Search words
	@SuppressWarnings("unchecked")
	public static JSONArray searchLevenshteinDistance(long sessionId, String symbolChars, int concentrationDepth, int orderDirection) throws Exception {
	...
	//Search words
	@SuppressWarnings("unchecked")
	public static JSONArray search(long sessionId, String symbolChars, String regex, int concentrationDepth, int orderDirection, String lang) throws Exception {
	...
	//Delete a word
	public static void deleteLang(long sessionId, String wordName, String lang) throws Exception {
		...
		if (rec==null) {
			...
			if (!((JSONObject) rec.get("fl")).containsKey(lang)) {
				...
				//Get all thoughts
				JSONArray thoughtList = ThoughtManager.list(sessionId, wordName, lang);
				
				//Delete all thoughts
				for(int i=0;i<thoughtList.size();i++) {
					
					ThoughtManager.delete(sessionId, wordName, ""+thoughtList.get(i), lang);
	...
	//Delete a word
	public static void delete(long sessionId, String wordName) throws Exception {
	...
	public static String firstTabLink(long sessionId, String word, String lang) throws Exception {
		...
		//Try to get the record
		JSONObject rec = Record.getNode(sessionId, "W["+word+"]");
		...
		return ExperienceManagerL1.getFirstTabLink(sessionId, "W["+word+"]", lang, true);

	}
	
	//Get the last word tab link
	public static String lastTabLink(long sessionId, String word, String lang) throws Exception {
		...
		return ExperienceManagerL1.getLastTabLink(sessionId, "W["+word+"]", lang, true);

	}
	
	//Get word tab link
	public static String getTabLink(long sessionId, String word, int position, String lang) throws Exception {
				
		JSONArray tab = ThoughtManager.list(sessionId, word, lang);
	...
	//Show word languages
	@SuppressWarnings("unchecked")
	public static JSONArray showLanguages(long sessionId, String word) throws Exception {
	...
	//Show word tab links
	public static JSONArray showTabLinks(long sessionId, String word, String lang) throws Exception {
		
		return ThoughtManager.list(sessionId, word, lang);

	}
	
	//Show symbol perception
	@SuppressWarnings("unchecked")
	public static JSONArray showSymbolPerception(long sessionId, String word, String lang) throws Exception {
		...
		for(int i=0;i<word.length();i++) {
			...
			JSONObject bd = Record.getNode(sessionId, k);
	...
	//Show symbol perception
	@SuppressWarnings("unchecked")
	public static JSONArray showTabLinkPerceptions(long sessionId, String word, String lang) throws Exception {
	...
	@SuppressWarnings("unchecked")
	public static JSONArray showThoughtPerceptions(long sessionId, String word, String lang) throws Exception {
		...
		String currentTabLink = ExperienceManagerL1.getFirstTabLink(sessionId, "W["+word+"]", lang, true);
		
		//Parse all tab links
		while (currentTabLink!=null) {
			
			JSONObject bd = Record.getNode(sessionId, currentTabLink);
			int perception = Integer.parseInt(""+bd.get("w"));
			maxPerception += perception;
			JSONObject obj = new JSONObject();
			...
			result.add(obj);
			currentTabLink = ExperienceManagerL1.getDownTabLink(sessionId, "W["+word+"]", currentTabLink, lang, true);
	...
	//Show word tab links
	public static JSONArray showTabLinks(long sessionId, String word) throws Exception {
		
		return ThoughtManager.list(sessionId, word);
