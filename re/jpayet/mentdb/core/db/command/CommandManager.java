...
public class CommandManager {
	...
	//Execute the command
	public static String executeAllCommands(boolean manageCommitOrRollback, SessionThread session, Vector<Vector<MQLValue>> inputVectorCmds, EnvManager env, String parent_pid, String current_pid) throws Exception {
		...
		try {
			...
			if (manageCommitOrRollback && !session.user.equals("")) {
				
				Database.execute_admin_mql(session, "commit");
			}
			...
			if (manageCommitOrRollback && !session.user.equals("")) {
				...
				Database.execute_admin_mql(session, "rollback");
			}
